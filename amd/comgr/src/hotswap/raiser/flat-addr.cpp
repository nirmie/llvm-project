...
int64_t decodeGlobalFlatOffset(int64_t RawOffset) {
  ...
  // (FLATInstructions.td), the signed 24-bit offset field shared by gfx12
  // GLOBAL and FLAT/SADDR forms. The MC operand can arrive as the raw encoded
  // field, so normalize it once here at the shared address decoder.
  constexpr unsigned GlobalFlatOffsetBits = 24;
  return SignExtend64<GlobalFlatOffsetBits>(static_cast<uint64_t>(RawOffset));
}

// Coerce an integer address into a global-AS pointer and apply a signed byte
// offset via a plain (non-inbounds) GEP. The ISA's signed offset can
// legitimately leave the base allocation (compiler-scheduled prefetches,
// negative strides); inbounds would turn that into UB.
...
Expected<int64_t> getGlobalFlatOffset(const DecodedInst &Di) {
  ...
  return createStringError(
      "transpiler: FLAT/GLOBAL opcode (opcode=" + Twine(Opc) +
...
Expected<FlatAddr> decodeGlobalLoadAddr(RaiseContext &Ctx,
  ...
  // SADDR form: saddr(SGPR64), vaddr(VGPR32), ... -- LLVM MC places the SGPR
  // first in the decoded operand order.
  if (Op.nSrcs() >= 2 && Op.isSrcReg(0) && Op.isSrcReg(1) &&
    ...
    // width: parseReg reports tuple VGPRs with width=1 on some subtargets;
    // readReg64 walks the sub0/sub1 graph itself, so trust the SGPR-vs-VGPR
    // discriminator above and let readReg64 enforce the 64-bit shape.
    Addr = Ctx.Regs.readReg64(Ctx.B, Op.srcReg(0));
  } else {
    return createStringError("transpiler: unrecognized " + Twine(DiagLabel) +
                             " operand shape (expected plain VGPR64 or SADDR "
                             "SGPR64+VGPR32): \"" +
                             printInst(Ctx.Mc, Di.Inst) + "\"");
  }
...
Expected<FlatAddr> decodeGlobalStoreAddr(RaiseContext &Ctx,
  ...
  if (Op.nSrcs() >= 3 && Op.isSrcReg(0) && Op.isSrcReg(1) && Op.isSrcReg(2) &&
    ...
    return createStringError("transpiler: unrecognized " + Twine(DiagLabel) +
                             " operand shape (expected plain VGPR+VGPR or SADDR "
                             "VGPR+VGPR+SGPR): \"" +
                             printInst(Ctx.Mc, Di.Inst) + "\"");
  }
