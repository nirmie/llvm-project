...
#include "hotswap/loader/pipeline-loader-shim.h"
...
static bool raiseAndCompileKernel(
  ...
  llvm::Expected<RaiseResult> RaisedOrErr =
      raiseToIR(Text.Bytes, SourceISA, KernelName, Meta, KernelOffset,
                ...
                /*ForceModrepDoubled=*/false, Text.Address, Text.ImageSections,
...
void collectTargetPrivateSegmentMetadata(
  ...
  for (llvm::StringRef KernelName : KernelNames) {
    ...
    // m0's CodeObjectInfo::create validates and rejects code objects whose
    // kernel descriptor it cannot read, so a KernelMeta here always has one.
