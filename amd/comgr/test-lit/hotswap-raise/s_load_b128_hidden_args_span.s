...
; RUN:   | %FileCheck %s
;
; Regression test for Bug-Id: 2026-05-26T18-08-17Z_qwen2.5-7b-instruct-005.
;
...
; ISA's implicit-args block previously failed with UnsupportedOpcode when a
...
;
; Fix (in handle-smem.cpp + source-hidden-args.cpp): the multi-dword loop
; falls back to an amdgcn_implicitarg_ptr load (at the dword's rebased offset)
; instead of emitting a hard failure when a dword has !Matched or IsGapSpan.
;
...
;   offset 16: hidden_global_offset_x (8 bytes)-> classified (via implicitarg_ptr)
;
...
;                       -> !Matched -> implicitarg_ptr at rebased offset 4
