...
; RUN:   | %FileCheck %s
;
; Regression test for Bug-Id: 2026-05-26T08-08-10Z_qwen2.5-7b-instruct-011.
...
; Also covers Bug-Id: 2026-05-26T15-20-57Z_qwen2.5-7b-instruct-011.
;
; s_mov_b64 with both immediate-zero and register-pair operands was failing
; with UnsupportedOpcode (format=operand-read) in 618 instances across 174
; rocSOLVER kernels (getf2_panel_kernel, getf2_npvt_panel_kernel,
; getf2_scale_update_kernel, stedc_mergePrepare_* kernels).
; The dominant patterns were `s_mov_b64 s[pair], 0` (zero-initialising a
; 64-bit SGPR pair before a conditional store) and `s_mov_b64 s[pair], s[pair]`
; (saving a pointer-pair into a scratch SGPR pair before a call).
;
; The fix is in handle-sop1.cpp: S_MOV_B64 dispatches to writeReg64, which
; reconstructs the i64 SGPR pair from the source operand (immediate or
; register pair) and stores it in the pair alloca.
;
