...
; RUN:     --emit-ir=copy_mat_cd_012_kernel 2>/dev/null | %FileCheck %s
;
; Regression test for Bug-Id: 2026-05-26T08-08-10Z_qwen2.5-7b-instruct-012
;
; The copy_mat<rocblas_complex_num<double>, double, true> kernel issues 10
; scratch_store_b128 instructions (SCRATCH_STORE_DWORDX4, 128-bit spills)
; with private_segment_fixed_size=0. Hotswap must accept the conservative
; 65536-byte-frame path for all 10 stores rather than failing with
; UnsupportedOpcode.
;
; Kernel: copy_mat<rocblas_complex_num<double>, double, true>
; Format: FLAT scratch (SCRATCH_STORE_DWORDX4 alias, mnemonic scratch_store_b128)

; CHECK-LABEL: define {{.*}}copy_mat_cd_012_kernel
...
; CHECK: store <4 x i32>
	...
	.text
	.globl	copy_mat_cd_012_kernel
	.p2align	8
	.type	copy_mat_cd_012_kernel,@function
copy_mat_cd_012_kernel:
	s_setreg_imm32_b32 hwreg(HW_REG_WAVE_MODE, 25, 1), 1
	s_load_b64 s[0:1], s[0:1], 0x0
	; Spill 10 × 128-bit complex-double temporaries via scratch_store_b128.
	...
	.p2align	6, 0x0
	.amdhsa_kernel copy_mat_cd_012_kernel
...
amdhsa.kernels:
  - .args:
    ...
    .name:           copy_mat_cd_012_kernel
    .private_segment_fixed_size: 0
    .sgpr_count:     2
    .symbol:         copy_mat_cd_012_kernel.kd
    .vgpr_count:     5
    .wavefront_size: 32
amdhsa.version: [1, 2]
...
