...
; RUN:     --emit-ir=s_and_saveexec_b32_kernel 2>/dev/null | %FileCheck %s
;
; Regression for Bug-Id 2026-05-26T10-21-53Z_qwen2.5-7b-instruct-007.
;
...
; earlier bug record.
;
; `s_and_saveexec_b32` on gfx1250 (wave32) was reported as UnsupportedOpcode
; (156 hits across 93 kernels).  The handler in handle-sop1.cpp recognises
; S_AND_SAVEEXEC_B32 and emits:
;   1. load old EXEC into dst SGPR (writeRegExecWidth)
;   2. AND old EXEC with src -> new_exec
...
; v_cmp_lt_u32 produces the mask via ballot.
; CHECK:       %vcmp = icmp ult i32 %tid, 16

; s_and_saveexec_b32 writes `old_exec AND src` to EXEC.
; The AND uses %saved_exec (the loaded EXEC) and the widened src SGPR.
